...
const post = ({postData}) =>{
    ...
    return(
          <div className="bg-primary text-b-n w-screen h-screen overflow-scroll">
          <div className="relative sm:h-1/4 md:h-1/3 w-full">
          {postData.featuredImage ? 
          <img className='absolute z-0 top-0 left-0 w-full h-60 md:h-80 object-cover' src={postData.featuredImage.node.sourceUrl} alt="cafe image" /> :
          <img className='absolute z-0 top-0 left-0 w-full h-60 md:h-80 object-cover' src='/Noimage.png' alt="cafe image"/>
          }                
          <div className="absolute top-20 md:top-28 left-1/2 transform -translate-x-1/2 p-3 w-1/2 rounded-lg border">
          <h1 className="font-title sm:text-lg md:text-3xl text-center px-3 py-5 bg-primary rounded-lg border">{postData.title}</h1>
          </div>
          </div>
          <div className="pt-72 md:pt-3 lg:pt-28 pb-10 bg-primary">
    ...
    <div className="mapbox-react relative ml-0 lg:ml-3 overflow-hidden">
    <ReactMapGL
      {...viewport}
      width="40vh"
      height="30vh"
...
export async function getStaticPaths(){
    ...
    return{
        paths: allPosts.edges.map(({node}) => `/east-vancouver/${node.slug}`) || [],
        fallback: false,
    }
